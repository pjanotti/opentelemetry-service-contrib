...
func (i *Input) processEvent(ctx context.Context, event Event) {
	...
	// We always need to get the provider name, it doesn't matter the rendering type.
	...
	if i.raw {
		rawEvent, renderRawErr := event.RenderRaw(i.buffer)
		if renderRawErr != nil {
			i.Logger().Error("Failed to render raw event", zap.Error(renderRawErr))
			return
		}
	...
	if publisher.Valid() {
		formattedEvent, renderFormattedErr := event.RenderFormatted(i.buffer, publisher)
		if renderFormattedErr == nil {
			formattedEvent.RemoteServer = remoteServer
			i.sendEvent(ctx, formattedEvent)
			return
		}

		i.Logger().Error("Failed to render simple event", zap.Error(renderFormattedErr))
	}

	simpleEvent, renderSimpleErr := event.RenderSimple(i.buffer)
	if renderSimpleErr != nil {
		i.Logger().Error("Failed to render simple event", zap.Error(renderSimpleErr))
		return
	}
